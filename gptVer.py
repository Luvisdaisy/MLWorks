...
def scrape_movie_list(url, headers):
...
def scrape_movie_reviews(movie_id, headers):
    # 爬取电影评论的用户ID和评分值
...
def main():
    ...
    headers_movie_list = {
        ...
        # headers for movie list scraping
    }
    headers_movie_reviews = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'User-Agent': ua.random,
        'Connection': 'keep-alive',
        ...
        # headers for movie reviews scraping
    }

    # 爬取电影列表
    movie_list = scrape_movie_list(
        'https://movie.douban.com/top250', headers_movie_list)
    ...
    for movie in movie_list:
        movie_id = movie[1]
        movie_reviews = scrape_movie_reviews(movie_id, headers_movie_reviews)
